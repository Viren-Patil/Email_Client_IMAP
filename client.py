...
# This variable maintains count of emails present in a mailbox
global mno

# Connect to the IMAP server on localhost:143
...
logged_in = [None, False] # logged_in means [mailbox selected, Boolean value]
selected_state = [None, False] # selected state means mailbox is selected 

while True:
	try:
		# Displaying prompt line according to the status of the user i.e. logged in and selected state both
		# or just in logged in state
		...
		# Login user
			...
			# Checking if the user wants to see server replies or the email client's replies.
		...
		# Fetching and reading of mails
			...
			if selected_state[1] == False:
				...
				for u in mails_to_be_displayed:
					command = read_header_mail(u) # function to fetch only headers
				...
				# Used tabulate library for formatting output in a table form	
		...
		# List all the mailboxes available for that particular user
		elif choice == 11:
		...
		# Send mail using smtp.gmail.com:465
		elif choice == 12:
		...
		# Send mail using SMTP on localhost:25
		elif choice == 13:
			...
			# Connecting to the localhost smtp server (postfix) on port 25
...
# Closing connection to localhost:143
clientSocket.close()